...
class State:
    ...
    def __init__(self, D=10 , Ron=.1, Roff=16, Mu=1e-10):
        ...
        self.W = [0] * values.totalParts
        self.WD = [0] * values.totalParts


    # callable gets W at a specific time
    def __call__(self, t):
        return self.W[values.getIndexFromT(t) - 1] if t != 0 else self.W[values.getIndexFromT(t)] # if at zero don't subtract 1
    ...
    def setWFromCharge(self, q, t):
        self.currentState = self.Mu * (self.Ron/self.D) * q(t)
...
class Current:
    ...
    CurrentAtPoint = [0] * values.totalParts       #This is for keeping track of the current at all points --Using a global is somewhat annoying attempt to fix later
